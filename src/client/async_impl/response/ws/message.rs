...
use wreq::ws::{self, CloseCode, CloseFrame, Utf8Bytes};
...
#[pyclass(subclass)]
#[derive(Clone)]
pub struct Message(pub ws::Message);
...
impl Message {
    ...
    pub fn data<'py>(&self, py: Python<'py>) -> Option<Bound<'py, PyAny>> {
        let bytes = match &self.0 {
            ws::Message::Text(text) => text.clone().into(),
            ws::Message::Binary(bytes) | ws::Message::Ping(bytes) | ws::Message::Pong(bytes) => {
                bytes.clone()
            }
            _ => return None,
        };
    ...
    pub fn text(&self) -> Option<&str> {
        if let ws::Message::Text(text) = &self.0 {
            Some(text)
        } else {
            None
        }
    ...
    pub fn binary<'py>(&self, py: Python<'py>) -> Option<Bound<'py, PyAny>> {
        if let ws::Message::Binary(data) = &self.0 {
    ...
    pub fn ping<'py>(&self, py: Python<'py>) -> Option<Bound<'py, PyAny>> {
        if let ws::Message::Ping(data) = &self.0 {
            BytesBuffer::new(data.clone()).into_bytes_ref(py).ok()
        } else {
            None
        }
    ...
    pub fn pong<'py>(&self, py: Python<'py>) -> Option<Bound<'py, PyAny>> {
        if let ws::Message::Pong(data) = &self.0 {
            BytesBuffer::new(data.clone()).into_bytes_ref(py).ok()
        } else {
            None
        }
    ...
    pub fn close(&self) -> Option<(u16, Option<&str>)> {
        if let ws::Message::Close(Some(s)) = &self.0 {
...
impl Message {
    ...
    pub fn text_from_json(py: Python, json: Json) -> PyResult<Self> {
        py.allow_threads(|| {
            ws::Message::text_from_json(&json)
                .map(Message)
                .map_err(Error::Request)
                .map_err(Into::into)
        })
    ...
    pub fn binary_from_json(py: Python, json: Json) -> PyResult<Self> {
        py.allow_threads(|| {
            ws::Message::binary_from_json(&json)
                .map(Message)
                .map_err(Error::Request)
                .map_err(Into::into)
        })
    ...
    pub fn from_text(text: PyBackedStr) -> Self {
        let msg = ws::Message::text(Utf8Bytes::from_bytes_unchecked(Bytes::from_owner(text)));
        Message(msg)
    }
    ...
    pub fn from_binary(data: PyBackedBytes) -> Self {
        let msg = ws::Message::binary(Bytes::from_owner(data));
        Message(msg)
    }
    ...
    pub fn from_ping(data: PyBackedBytes) -> Self {
        let msg = ws::Message::ping(Bytes::from_owner(data));
        Message(msg)
    }
    ...
    pub fn from_pong(data: PyBackedBytes) -> Self {
        let msg = ws::Message::pong(Bytes::from_owner(data));
        Message(msg)
    }
    ...
    pub fn from_close(code: u16, reason: Option<PyBackedStr>) -> Self {
        ...
        let msg = ws::Message::close(CloseFrame {
            code: CloseCode(code),
            reason,
        });
        Message(msg)
