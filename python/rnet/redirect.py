...
class Policy:
    ...
    def limited(max: int) -> "Policy":
        ...
        Example:
            ```python
            policy = rnet.redirect.Policy.limited(5)
            client = rnet.Client(redirect=policy)
            ```
        """
        ...
    ...
    def none() -> "Policy":
        ...
        Example:
            ```python
            policy = rnet.redirect.Policy.none()
            client = rnet.Client(redirect=policy)
            ```
        """
        ...
    ...
    def custom(callback: Callable[["Attempt"], "Action"]) -> "Policy":
        ...
        Example:
            ```python
            def custom_policy(attempt: rnet.redirect.Attempt) -> rnet.redirect.Action:
            ...
            policy = rnet.redirect.Policy.custom(custom_policy)
            client = rnet.Client(redirect=policy)
            ```
        """
        ...
